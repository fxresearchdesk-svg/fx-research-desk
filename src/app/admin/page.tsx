...
export default function AdminPage() {
    ...
    return (
      <main className="min-h-screen bg-[#0a0a0a] text-white flex items-center justify-center px-6">
        <form
          ...
          className="w-full max-w-md bg-slate-900/50 border border-slate-800 rounded-2xl p-8"
        >
          ...
          <input
            ...
            className="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-3 mb-4 text-white"
          />
          {message && <p className="text-red-400 text-sm mb-4">{message}</p>}
          <button
            type="submit"
            disabled={loading || !password}
            className="w-full bg-emerald-500 hover:bg-emerald-400 text-black font-bold py-3 rounded-lg disabled:opacity-50"
          >
            {loading ? "Verifying..." : "Login"}
          </button>
          <Link
            href="/"
            className="inline-flex items-center gap-2 text-slate-500 text-sm mt-6 hover:text-emerald-400"
          >
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white">
      <header className="border-b border-slate-800 bg-[#0a0a0a]/95 backdrop-blur-xl sticky top-0 z-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-10 space-y-10">
        ...
        <section className="bg-slate-900/30 border border-slate-800 rounded-2xl p-6">
          ...
          <button
            ...
            disabled={publishing}
            className="inline-flex items-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-black font-bold px-6 py-2.5 rounded-lg disabled:opacity-50"
          >
        ...
        <section className="bg-slate-900/30 border border-slate-800 rounded-2xl p-6">
          ...
          <form onSubmit={handleSaveSignal} className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            ...
            <button
              type="submit"
              className="bg-emerald-500 text-black font-bold rounded-lg px-4 py-2"
            >
          ...
          <div className="space-y-2">
            {signals.map((signal) => (
              <div
                key={signal.id}
                className="flex flex-wrap items-center justify-between gap-3 bg-slate-950/50 border border-slate-800/50 rounded-lg px-4 py-3"
              >
                <div className="text-sm">
                  <span className="font-semibold text-white">{signal.pair}</span>{" "}
                  <span className="text-emerald-400">{signal.direction}</span> · Entry{" "}
        ...
        <section className="bg-slate-900/30 border border-slate-800 rounded-2xl p-6">
          ...
          <form onSubmit={handleSaveTestimonial} className="space-y-4 mb-6">
            ...
            <button
              type="submit"
              className="bg-emerald-500 text-black font-bold rounded-lg px-6 py-2"
            >
          ...
          <div className="space-y-2">
            {testimonials.map((t) => (
              <div
                key={t.id}
                className="flex flex-wrap items-start justify-between gap-3 bg-slate-950/50 border border-slate-800/50 rounded-lg px-4 py-3"
              >
                <div className="text-sm max-w-xl">
                  <p className="text-slate-300 italic mb-1">&ldquo;{t.quote}&rdquo;</p>
