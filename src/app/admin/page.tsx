...
export default function AdminPage() {
  ...
  if (!authenticated) {
    return (
      <main className="min-h-screen bg-[#F8F9FA] text-[#111827] flex items-center justify-center px-6">
        <form
          onSubmit={handleLogin}
          className="w-full max-w-md bg-[#FFFFFF] border border-[#E5E7EB]  p-8"
        >
          <h1 className="text-2xl font-bold mb-2">Admin Dashboard</h1>
          <p className="text-[#9CA3AF] text-sm mb-6">Enter admin password to continue</p>
          <input
            type="password"
            value={password}
            onChange={(e) => setPassword(e.target.value)}
            placeholder="Admin password"
            className="w-full bg-[#F8F9FA] border border-[#E5E7EB]  px-4 py-3 mb-4 text-[#111827]"
          />
          {message && <p className="text-red-400 text-sm mb-4">{message}</p>}
          <button
            type="submit"
            disabled={loading || !password}
            className="w-full border border-[#E5E7EB] bg-transparent text-[#B8956A] hover:bg-[#B8956A] hover:text-white font-bold py-3  disabled:opacity-50"
          >
  ...
  return (
    <main className="min-h-screen bg-[#F8F9FA] text-[#111827]">
      <header className="border-b border-[#E5E7EB] bg-[#F8F9FA]/95 backdrop-blur-xl sticky top-0 z-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-10 space-y-10">
        {message && (
          <div className="bg-[#B8956A] text-white/10 border border-emerald-500/30 text-[#B8956A] px-4 py-3  text-sm">
            {message}
          </div>
        )}

        {/* Stats */}
        <section className="bg-[#FFFFFF] border border-[#E5E7EB]  p-6">
            ...
            {[
              ...
              <div key={field.key}>
                ...
                <input
                  ...
                  className="w-full bg-[#F8F9FA] border border-[#E5E7EB]  px-3 py-2 text-[#111827]"
                />
              </div>
            ))}
          </div>
          <button
            ...
            disabled={publishing}
            className="inline-flex items-center gap-2 border border-[#E5E7EB] bg-transparent text-[#B8956A] hover:bg-[#B8956A] hover:text-white font-bold px-6 py-2.5  disabled:opacity-50"
          >
        ...
        <section className="bg-[#FFFFFF] border border-[#E5E7EB]  p-6">
          ...
          <form onSubmit={handleSaveSignal} className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            <input
              ...
              placeholder="Pair"
              className="bg-[#F8F9FA] border border-[#E5E7EB]  px-3 py-2"
            />
            <select
            ...
            <button
              type="submit"
              className="bg-[#B8956A] text-white font-bold  px-4 py-2"
            >
          ...
          <div className="space-y-2">
            {signals.map((signal) => (
              <div
                key={signal.id}
                className="flex flex-wrap items-center justify-between gap-3 bg-[#F8F9FA]/50 border border-[#E5E7EB]  px-4 py-3"
              >
                <div className="text-sm">
                  <span className="font-semibold text-[#111827]">{signal.pair}</span>{" "}
        ...
        <section className="bg-[#FFFFFF] border border-[#E5E7EB]  p-6">
          ...
          <form onSubmit={handleSaveTestimonial} className="space-y-4 mb-6">
            ...
            <textarea
              ...
              placeholder="Quote"
              rows={3}
              className="w-full bg-[#F8F9FA] border border-[#E5E7EB]  px-3 py-2"
            />
            <input
            ...
            <button
              type="submit"
              className="bg-[#B8956A] text-white font-bold  px-6 py-2"
            >
          ...
          <div className="space-y-2">
            {testimonials.map((t) => (
              <div
                key={t.id}
                className="flex flex-wrap items-start justify-between gap-3 bg-[#F8F9FA]/50 border border-[#E5E7EB]  px-4 py-3"
              >
                <div className="text-sm max-w-xl">
                  <p className="text-[#9CA3AF] italic mb-1">&ldquo;{t.quote}&rdquo;</p>
